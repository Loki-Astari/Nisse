#ifndef THORSANVIL_NISSE_PYNTHTTP_STREAMINPUT_H
#define THORSANVIL_NISSE_PYNTHTTP_STREAMINPUT_H
...
class StreamBufInput: public std::streambuf
{
    ...
    public:
        ...
        StreamBufInput(std::istream& stream, EncodingChunked, Complete&& complete = [](){});
...
class StreamInput: public std::istream
{
    ...
    public:
        ...
        StreamInput(std::istream& stream, EncodingChunked chunked)
            : std::istream(nullptr)
            , buffer(stream, chunked)
        {
            rdbuf(&buffer);
        }
