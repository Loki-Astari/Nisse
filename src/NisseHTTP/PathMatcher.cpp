...
std::string PathMatcher::decode(std::string_view matched)
{
    ...
    for (std::size_t loop = 0; loop < matched.size(); ++loop)
    {
        ...
        {
            unsigned char c1 = std::toupper(matched[loop + 1]);
            int  v1 = (c1 >= '0' && c1 <= '9') ? c1 - '0' : c1 - 'A' + 10;
            unsigned char c2 = std::toupper(matched[loop + 2]);
...
NISSE_HEADER_ONLY_INCLUDE
void PathMatcher::removePrefix(std::string_view& v, std::size_t size)
{
    v.remove_prefix(std::min(size, std::size(v)));
}
...
bool PathMatcher::checkPathMatch(MatchInfo const& pathMatchInfo, std::string_view path, Request& request, Response& response)
{
    ...
    removePrefix(path, pathMatchInfo.matchSections[0].size());
    ...
    for (std::size_t loop = 1; loop < pathMatchInfo.matchSections.size(); ++loop)
    {
        ...
        removePrefix(path, find);
        removePrefix(path, pathMatchInfo.matchSections[loop].size());
    }
